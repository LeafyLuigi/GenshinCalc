...
var setLSItem = (LSName, value) => {
	if (typeof(value) != "string") {
		throw new Error("The value was not a string.");
	}
	try {
		localStorage.setItem(LSName,value);
	}
	catch {
		throw new Error("localStorage is full. Nothing was set. This will very likely cause problems.");
	}
}
var updateLSItem = (LSName,key,newValue,mustExist=false) => {
	...
	if(item !== null) {
		...
		throw new Error("Could not set the item as it didn't exist.");
	}
}
...
var importLS = (sure=false,importData) => {
	...
	if(typeof(importData) == "string") {
		try {
			...
			throw new Error("Could not parse import data. Nothing has been set.");
		}
	...
	if(Array.isArray(data)) {
		throw new Error("Arrays are not allowed to be imported.");
	}
	if(sure !== "true") {
